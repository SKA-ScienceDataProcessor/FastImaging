...
TEST(GridderOversampledGridding, equal)
{
    ...
    std::pair<MatStp<cx_real_t>, MatStp<cx_real_t>> result = convolve_to_grid(triangle, support, image_size, uv, vis, kernel_exact, oversampling, false, true, false, false);

    EXPECT_TRUE(arma::accu(arma::real(static_cast<arma::Mat<cx_real_t>>(result.first))) - arma::accu(arma::real(vis)) < tolerance);
}
