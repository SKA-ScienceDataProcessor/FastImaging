...
TEST(GridderBoundsChecking, vis_grid)
{
    ...
    std::pair<arma::cx_mat, arma::mat> grid = convolve_to_grid(TopHat(half_base_width), support, image_size, bad_uv, vis, kernel_exact, oversampling, pad, normalize);
    ...
    std::pair<arma::cx_mat, arma::mat> good_grid = convolve_to_grid(TopHat(half_base_width), support, image_size, good_uv, vis, kernel_exact, oversampling, pad, normalize);

    vis = arma::ones<arma::cx_mat>(mixed_uv.n_rows);
    std::pair<arma::cx_mat, arma::mat> mixed_grid = convolve_to_grid(TopHat(half_base_width), support, image_size, mixed_uv, vis, kernel_exact, oversampling, pad, normalize);
