...
static void imager_test_benchmark(benchmark::State& state)
{
    ...
    while (state.KeepRunning()) {
        benchmark::DoNotOptimize(result = stp::image_visibilities(kernel_func, residual_vis, input_uvw, cfg.image_size, cfg.cell_size, cfg.kernel_support, cfg.kernel_exact, cfg.oversampling));
    }
}
