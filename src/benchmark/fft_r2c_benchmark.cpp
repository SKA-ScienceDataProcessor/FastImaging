...
static void fft_r2c_test_benchmark(benchmark::State& state)
{
    stp::fft_routine r_fft = (stp::fft_routine)state.range(0);

    int image_size = pow(2, double(state.range(1) + 19) / 2.0);
    std::string wisdom_filename = wisdom_path + "WisdomFile_rof" + std::to_string(image_size) + "x" + std::to_string(image_size) + ".fftw";
    ...
    // Compute FFT of second matrix (beam)
    if (cfg.kernel_exact) {
        // Shift beam if kernel_exact is true
        stp::fftshift(gridded_data.second, false);
    }
    ...
    case stp::FFTW_ESTIMATE_FFT:
        ...
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(fft_result_beam);
            fft_result_beam = stp::fft_fftw_r2c(gridded_data.second, r_fft);
            benchmark::ClobberMemory();
        }
        break;
    case stp::FFTW_WISDOM_FFT:
        ...
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(fft_result_beam);
            fft_result_beam = stp::fft_fftw_r2c(gridded_data.second, r_fft, wisdom_filename);
            benchmark::ClobberMemory();
        }
        break;
    ...
    if (r_fft == stp::FFTW_MEASURE_FFT || r_fft == stp::FFTW_PATIENT_FFT) {
        arma::Mat<real_t> input(arma::size(static_cast<arma::Mat<real_t>&>(gridded_data.second)));
        arma::Mat<cx_real_t> output(input.n_rows / 2 + 1, input.n_cols);
...
BENCHMARK(fft_r2c_test_benchmark)
