...
```sh
./reduce [-d] [-l] [-p] <input-file-json> <input-file-npz> <output-file-json> <output-file-npz>
```
...
-l, --log   | optional | Enable logger (write to standard output and file).
-p, --print | optional | Print islands found to standard output.

Example:
```sh
$ cd build-directory/reduce
$ ./reduce fastimg_oversampling_config.json simdata_nstep10.npz detected_islands.json -d -l -p
```
...
```sh
./run_sourcefind  [-l] [-p] <input-file-json> <input-file-npz> <output-file-json> <output-file-npz>
```
...
```sh
$ cd build-directory/reduce
$ ./run_sourcefind fastimg_oversampling_config.json dirty_image.npz detected_islands.json -l -p
```
...
islands = stp_python.source_find_wrapper(cpp_img, detection_n_sigma, 
            ...
            ccl_4connectivity, generate_labelmap, source_min_area,
...
$ cd path/to/build/directory
$ cd reduce
$ valgrind --tool=callgrind --separate-threads=yes ./reduce fastimg_oversampling_config.json simdata_nstep10.npz detected_islands.json -d -l -p
