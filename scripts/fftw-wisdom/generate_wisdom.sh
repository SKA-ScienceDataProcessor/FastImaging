...
for IMGSIZE in $SIZES ; do
	if [ $IMGSIZE -le $MAX_SIZE ] ; then
		...
		FFTWSTR2="${FFTWSTR2} ${TYPE_CX_1D_FFT}${IMGSIZE}"
		FFTWSTR3="${FFTWSTR3} ${TYPE_R2C_2D_FFT}${IMGSIZE}x${IMGSIZE}"
		FFTWSTR4="${FFTWSTR4} ${TYPE_CX_2D_FFT}${IMGSIZE}x${IMGSIZE}"
	fi
done

echo "${EXECUTABLE} -n -m -T ${NPROC} -o ${OUTDIR}/${OUTFILE} ${FFTWSTR1} ${FFTWSTR2} ${FFTWSTR3} ${FFTWSTR4}"
${EXECUTABLE} -n -m -T ${NPROC} -o ${OUTDIR}/${OUTFILE} ${FFTWSTR1} ${FFTWSTR2} ${FFTWSTR3} ${FFTWSTR4}
